...
class TMDB {
	 ...
	 * 	@return Movie[]
	 */
	public function getDiscoverMovies($page = 1, $genre, $sort, $year) {
		...
		$result = $this->_call('discover/movie', '&page='. $page . $sort_query . $genre_query . $year_query);
	 ...
	 * 	@return string
	 */
	private function _callLocal( $id ) {
		$post = get_post( $id );
		$results = get_transient( 'watu_'.$post->post_name.'_transient_for_' . $post->post_name );
		if ( false === $results ) {
	    $results = get_the_content( $id );
	    set_transient( 'watu_'.$post->post_name.'_transient_for_' . $post->post_name, $results, 7 * DAY_IN_SECONDS );
		}
	...
	public function getMovie($idMovie) {
		return new Movie($this->_callLocal($idMovie));
	}
	...
	public function getPerson($idPerson) {
		return new Person($this->_callLocal($idPerson));
	}
